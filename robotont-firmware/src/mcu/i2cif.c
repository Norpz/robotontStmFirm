...
void i2cif_memoryRead(I2C_HandleTypeDef *i2c_handler, uint16_t slave_addr, uint16_t mem_addr,
                      uint16_t mem_size, uint8_t *ptr_data, uint16_t data_size, uint32_t timeout_ms)
{
    // Start I2C communication
    if (HAL_I2C_Mem_Read(i2c_handler, slave_addr, mem_addr, mem_size, ptr_data, data_size, timeout_ms) != HAL_OK)
    {
